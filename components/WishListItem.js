...
const WishListItem = (props) => {
  ...
  const deleteWishListItem = (id) => {
    ...
    axios
    .post(API_URL+"/wishlist/delete_wishlist.php" ,
    {
      productid : id,
      studentid : studentid,
  ...
  const addCartItem = (id) => {
    axios
    .post(API_URL+"/wishlist/wishlist_to_cart.php" ,
    {
      productid : id,
      studentid : studentid,
    })
  ...
  const update_wishlist_quantity = (value,id) =>{
    
    setQuantity(value);
    var producttotal=props.cost*value;
    axios
    .post(API_URL+"/wishlist/update_wishlist_quantity.php",{
      quantity  : value,
      studentid : studentid,
      productid : id,
      producttotal : producttotal,
    ...
    <div>
                      ...
                      <div className="w-full xl:w-auto px-4 mb-6 xl:mb-0 mt-6 xl:mt-0"> 
                        <div className="lg:ml-0 md:ml-16 flex items-center">
                          ...
                          <input value={quantity} onChange={(e) => (update_wishlist_quantity(e.target.value,props.id))} className="w-16 px-3 py-2 text-center placeholder-gray-400 text-black bg-blue-50 border-2 border-blue-400 outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 rounded-xl" type="number" placeholder="1"/>
                          <button onClick={()=> deleteWishListItem(props.id)} className='lg:ml-5 md:ml-10 ml-5 bg-blue-500  hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-xl'> Delete </button>
                          <button onClick={()=> addCartItem(props.id)} className='lg:ml-5 md:ml-10 ml-5 bg-blue-500  hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-xl'> Move to Cart</button>
                        </div>
                      </div>
