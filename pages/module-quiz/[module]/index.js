...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (!questionData && courseSlug && isStudentLoggedIn() === true) {
      axios
        .post(API_URL + "module_quiz/get_questions.php", { module: courseSlug, user_id: StudentData().ID })
        .then((response) => {
          setQuizInit(true);
  ...
  function submitAnswers() {
    setPageLoading(true);
    axios
      .post(API_URL + "module_quiz/get_questions.php", { user_id: StudentData().ID, module: course, answers: answersData })
      .then((response) => {
        setQuizInit(false);
        setTimerActive(false);
        setQuestionData(response.data.questionsData);
  ...
  function tryAgain(quiz_course_id) {
    setPageLoading(true);
    axios
      .post(API_URL + "module_quiz/retry_quiz.php", { course_id: quiz_course_id, user_id: StudentData().ID })
      .then((response) => {
              ...
              <div className="p-3 lg:p-10">
                ...
                <div className="py-5 px-2 lg:px-8 space-y-3">
                  <div className="option1">
                    ...
                    <span class="ml-2 text-gray-700  text-lg font-serif capitalize">{questionData[questionIndex].OptionA}</span>
                  </div>
                  <div className="option2">
                    <input onClick={() => setAnswer("B")} name={questionData[questionIndex].ID} type="radio" class="form-radio h-5 w-5 text-indigo-600" />
                    <span class="ml-2 text-gray-700  text-lg font-serif capitalize">{questionData[questionIndex].OptionB}</span>
                  </div>
                  <div className="option3">
                    <input onClick={() => setAnswer("C")} name={questionData[questionIndex].ID} type="radio" class="form-radio h-5 w-5 text-indigo-600" />
                    <span class="ml-2 text-gray-700  text-lg font-serif capitalize">{questionData[questionIndex].OptionC}</span>
                  </div>
                  <div className="option4">
                    <input onClick={() => setAnswer("D")} name={questionData[questionIndex].ID} type="radio" class="form-radio h-5 w-5 text-indigo-600" />
                    <span class="ml-2 text-gray-700  text-lg font-serif capitalize">{questionData[questionIndex].OptionD}</span>
                  </div>
                </div>
              </div>
          ...
          {!quizStatus && questionData.length > 0 ? (
            <div className="flex flex-col lg:flex-row justify-center lg:justify-between items-center">
              <div className="flex flex-col lg:flex-row justify-center items-center">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-36 w-36 lg:h-16 lg:w-16 text-green-600 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
